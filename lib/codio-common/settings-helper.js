window.codioAssessmentSettingsHelper = function () {
  return {
    ...
    renderPoints: (container, settings) => {
      const points = isNaN(settings.points) ? 20 : settings.points
    ...
    renderAttempts: (container, settings) => {
      const {maxAttemptsCount} = settings
    ...
    renderRationale: (container, settings) => {
      const {showGuidanceAfterResponseOption, guidance} = settings
